package com.searchengine.pagerank.test;
...
public class TestDdbConnector {
	...
	public List<WordIdfDdbDocument> getAllWordIdf() {
		DynamoDBScanExpression expr =
			new DynamoDBScanExpression().withConsistentRead(false);
		DynamoDBMapperConfig config =
			DynamoDBMapperConfig.builder().withPaginationLoadingStrategy(
					PaginationLoadingStrategy.ITERATION_ONLY).build();
		return d_mapper.parallelScan(WordIdfDdbDocument.class, expr, 4, config);
	}
	...
	public WordIdfDdbDocument getwordidf(String word) {
		return d_mapper.load(WordIdfDdbDocument.class, word);
	}
